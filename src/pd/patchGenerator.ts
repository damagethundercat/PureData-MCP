export type DemoPatchName = "basic_sine" | "drone" | string;

export interface GenerateDemoPatchOptions {
  patchId: string;
  patch: DemoPatchName;
  port: number;
  editPort: number;
...
export function generateDemoPatch(options: GenerateDemoPatchOptions): string {
  const { patchId, port, editPort, frequency, amplitude } = options;

  return `#N canvas 120 80 760 520 12;
#X obj 40 40 loadbang;
#X msg 40 70 MCP_READY ${patchId} port ${port};
#X obj 40 100 print;
#X obj 280 40 netreceive -f ${port};
...
#X msg 520 245 ${frequency};
#X msg 560 245 ${amplitude};
#X msg 600 245 1;
#X obj 40 415 netreceive -f ${editPort};
